...
> **重要:** このサーバーは、バックエンドの Codex が ChatGPT / OpenAI にログイン済みでないと、`/v1/threads` などの Codex を呼び出すエンドポイントが失敗します。**サーバーを本格的に使う前に、必ず一度この Codex ログインを済ませてください。** ログイン手順は下記「ログイン手順」を参照してください。

デフォルトは `CODEX_AUTH_MODE=chatgpt` です。既存の Codex 認証セッションがあれば再利用し、初回は Device Code login を使用します。

### ログイン手順 (ChatGPT / Device Code)

1. まずビルドしておきます(未ビルドの場合)。

   ```bash
   docker compose build
   ```

2. Codex ログインコマンドを実行します。`run --rm` を使うのは、ログインだけを一時コンテナで行い、認証情報を永続 volume (`codex-auth`) に書き込むためです。

   ```bash
   docker compose run --rm codex-api python -m cli.codex_auth login
   ```

3. すると以下のように **Verification URL** と **Code** が表示されます。
...
4. **PC やスマホのブラウザで Verification URL を開き、表示された Code を入力**して、ChatGPT アカウントでログインを承認します。

5. 承認が完了すると、コマンド側に `Login successful.` と表示されて終了します。認証情報は named volume `codex-auth` (`/home/codex/.codex`) に保存され、コンテナを再起動・再ビルドしても保持されます。

6. ログイン状態は次のコマンドで確認できます(token や email は表示しません)。
...
7. ログイン後にサーバーを起動(または再起動)します。

   ```bash
   docker compose up -d
   ```

   起動後、`GET /health` の `"authenticated": true` でも認証済みか確認できます。
...
### トラブルシュート: `{"detail":"Internal server error."}` が返る
...
- **バックエンド Codex が起動に失敗している**: 最も多い原因です。`CODEX_HOME`(コンテナ内 `/home/codex/.codex`)ディレクトリが存在しない、あるいは Codex ランタイムがログインできず、FastAPI の起動処理(lifespan)自体が失敗している状態です。
  ...
  2. 上記「ログイン手順」に従って Codex ログインを完了させます。
  3. サーバーを再起動します。
...
表示された Verification URL / Code でブラウザから ChatGPT ログインを完了してください。詳しい手順・確認方法・トラブルシュート(`{"detail":"Internal server error."}` が返る場合を含む)は上記「[Codex Authentication](#codex-authentication)」セクションを参照してください。
